...
function Menus({children}) {
    ...
    return (
        <MenusContex.Provider value={{openId, open, close, position, setPosition}}>
            <div>{children}</div>
...
function List({id, children}) {
    const {openId, position, setPosition} = useContext(MenusContex)
...
function Button({children, icon, onClick}) {
    ...
    return <li>
        <StyledButton ref={ref} onClick={handleClick}>{icon}<span>{children}</span></StyledButton>
    </li>
}
