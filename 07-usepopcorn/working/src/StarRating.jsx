...
export function StarRating({
    ...
    const [tempRating, setTempTaring] = useState(0)
    ...
    return <div style={containerStyle}>
        <div style={starContainerStyle}>
            {Array.from({length: maxRating}, (_, i) =>
                <Star
                    onRate={() => handleRating(i + 1)} onHoverIn={() => setTempTaring(i + 1)}
                    onHoverOut={() => setTempTaring(0)} full={rating >= i + 1 || tempRating >= i + 1}
                    key={i} color={color} size={size}/>)}
        </div>
